...
class DatasourceResourceIT {
  ...
  @Test
  @Transactional
  fun `test partial update datasource entity`() {
    datasourceRepository.saveAndFlush(datasource)
    val databaseSizeBeforeUpdate = datasourceRepository.findAll().size

    val partialUpdatedDatasource = Datasource().apply {
      id = datasource.id
      title = UPDATED_TITLE
    }

    mockMvc.perform(
      patch("/api/datasources")
        .contentType(APPLICATION_JSON)
        .content(convertObjectToJsonBytes(partialUpdatedDatasource))
    ).andExpect(status().isOk)
